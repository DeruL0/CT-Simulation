...
class StructureModifier:
    ...
    def generate_lattice(
        ...
        Args:
            config: LatticeConfig with type, density, cell size, and shell settings
        ...
        # Shape: (X,) + (Y,) + (Z,) -> expand dims for broadcasting
        Xs = x[:, None, None] * freq
        Ys = y[None, :, None] * freq
        Zs = z[None, None, :] * freq
        ...
        if config.preserve_shell and inner_mask is not None:
            # Preserve shell: keep original outside inner_mask
    ...
    def _compute_inner_mask(self, shell_thickness_mm: float, use_gpu: bool = True) -> np.ndarray:
        ...
        if use_gpu and HAS_GPU:
            ...
            # CuPy requires brute_force=True for iterations > 1
    ...
    def generate_random_voids(
        ...
        Args:
            config: DefectConfig with shape, density, size distribution, and shell settings
        ...
        # Pre-generate all random sizes and positions for batch processing
        ...
        # Adjust centers to ensure defects don't go outside bounds
        ...
        # Vectorized placement with batch overlap checking
        placed_centers = []
        placed_radii = []
        candidate_idx = 0
        ...
        while current_void_volume < target_void_volume and candidate_idx < max_candidates:
            ...
            # Vectorized overlap check (much faster than Python loop)
            ...
            # Place the defect (only within work_mask if shell preservation)
    ...
    def _add_ellipsoid_void_masked(
        self,
        center: Tuple[float, float, float],
        radii: Tuple[float, float, float],
        work_mask: Optional[np.ndarray] = None
